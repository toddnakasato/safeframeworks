/**
 * Chat builder for this renderer's SafeChat — config-driven chat component.
 *
 * Framework-agnostic DOM port of the react SafeChat implementation:
 * message bubbles, text input, send button, quick actions.
 *
 * Structure + data-* attributes ONLY — paint lives in safestyles.
 * Owns local message list (appends user messages on send) in the builder
 * closure; only the message list is rebuilt on send so the input keeps
 * focus/value. Events: "send" (user typed message), "action" (quick action
 * clicked). Same data-role markup as the react JSX implementation, so one
...
/** Build the chat into a container. Returns the root for removal. */
export function createSafeChat(container: HTMLElement, config: ConfigBase, onEvent?: OnSafeEvent): HTMLElement {
    ...
    /* Messages — only this region is rebuilt on send */
    ...
    if (quickActions.length > 0) {
        ...
        for (const action of quickActions) {
            ...
            btn.onclick = () => onEvent?.(createSafeEvent("chat", "action", { label: action.label }));
...
/**
 * Scan-and-mount for server-rendered hosts (Astro): build a chat in every
 * div[data-chat-config] not yet mounted.
 */
