/**
 * Button builder for this renderer's SafeButton — clickable action trigger,
 * single or grouped.
 *
 * Framework-agnostic DOM port of the react SafeButton:
 *   Single mode: one button with icon, label, iconRight, suffix, description.
 *   Group mode (children or groupVariant present): container wrapping child
 *   buttons. Group variants: toolbar, toggle, stepper, pagination.
 *
 ...
 * covers both. Icons: kebab-case lucide names resolve to inline SVG;
 * anything else (emoji, glyphs) renders as text — same span wrappers.
 */
...
/** span[data-role] holding either a resolved lucide SVG or the raw text (react renders text). */
...
/** Build the button (single or group) into a container. Returns the root for removal. */
...
/**
 * Scan-and-mount for server-rendered hosts (Astro): build a button in every
 * div[data-button-config] not yet mounted.
 */
