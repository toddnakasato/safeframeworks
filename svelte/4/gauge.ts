/**
 * Gauge builder for this renderer's SafeGauge — D3 radial KPI gauge.
 *
 * Framework-agnostic DOM port of the react JSX implementation:
 * background arc + animated value arc + optional target line + value/label.
 *
 * Structure + data-* attributes mirror react exactly; the only inline styles
 * are those react itself sets inline (svg sizing). Events: "click".
 */
import * as d3 from "d3";
import type { ConfigBase, OnSafeEvent } from "../../../safecontracts/src/contracts";
import { createSafeEvent } from "../../../safecontracts/src/contracts";

/** Build the gauge into a container. Returns the root for removal. */
export function createSafeGauge(container: HTMLElement, config: ConfigBase, onEvent?: OnSafeEvent): HTMLElement {
    ...
    root.onclick = () => onEvent?.(createSafeEvent("gauge", "click", { value }));
...
/**
 * Scan-and-mount for server-rendered hosts (Astro): build a gauge in every
 * div[data-gauge-config] not yet mounted.
 */
