/**
 * Card builder for this renderer's SafeCard — record display card.
 *
 * Framework-agnostic DOM port of the react JSX implementation:
 *   rows mode    metadata.rows (RowDef[]) lays out cells per row
 *   fields mode  schema fields from the first DataSource render label/value rows
 *
 * Structure + data-* attributes ONLY — paint lives in safestyles.
 * Events: "click" (card body), "back" (back label). Same data-role markup as
...
/** Build the card into a container. Returns the root for removal. */
...
/**
 * Scan-and-mount for server-rendered hosts (Astro): build a card in every
 * div[data-card-config] not yet mounted.
 */
