<script>
  ...
  let activeComponent = DEFAULT_COMP;
  let activeVariation = Object.keys(SAMPLES[DEFAULT_COMP] ?? {}).sort()[0] ?? null;
  let tickets = [];
  let ticketView = null;
  let proofView = false;
  ...
  let proofProgress = null;
  let proofToast = null;
  let orbitorView = false;
  let activeOrbitor = null;
  let orbitorTail = null;
  let orbitorLoading = false;
...
<div class="viewer">
  <div class="sidebar">
    ...
    <div style="display:flex;align-items:center;gap:8px;margin-top:16px">
      <button class="section-label" style="margin-top:0;background:none;border:none;padding:0;cursor:pointer;text-align:left;flex:1;text-decoration:{orbitorView ? 'none' : 'underline'}" on:click={() => showOrbitors(activeOrbitor)}>ORBITORS</button>
      <button style="font-size:10px;font-weight:600;padding:1px 8px;border-radius:3px;border:1px solid var(--sd-border, #d1d5db);background:var(--sd-surface-base, #fff);color:var(--sd-text, #1a1a1a);cursor:pointer" on:click={() => wakeOrbitor(null)}>Wake All</button>
    </div>
    <select class="dropdown" value={orbitorView ? (activeOrbitor ?? "All") : "All"} on:change={(e) => showOrbitors(e.target.value === "All" ? null : e.target.value)}>
      <option value="All">All</option>
      {#each ORBITOR_NAMES as n}<option value={n}>{n}</option>{/each}
    </select>

    <div class="section-label" style="margin-top:16px">PROOFS</div>
    <select class="dropdown" value={activeProof && activeProof !== "__none__" ? activeProof : "All"} on:change={(e) => showProofs(e.target.value === "All" ? null : e.target.value)}>
      <option value="All">All</option>
    ...
    <div style="display:flex;flex-direction:column;gap:6px;padding:0 2px;margin-top:16px">
      <div>
        <button class="dropdown-label" style="background:none;border:none;padding:0;cursor:pointer;text-align:left;width:100%;text-decoration:{(!proofView && !ticketView && !orbitorView) ? 'none' : 'underline'}" on:click={() => activeComponent && (activeVariation ? selectVariation(activeComponent, activeVariation) : selectComponent(activeComponent))}>Component</button>
        <select class="dropdown" value={activeComponent} on:change={(e) => selectComponent(e.target.value)}>
          {#each componentNames as n}<option value={n}>{n}</option>{/each}
        </select>
      </div>
      <div>
        <label class="dropdown-label">Variation</label>
        <select class="dropdown" value={activeVariation} on:change={(e) => selectVariation(activeComponent, e.target.value)} disabled={!activeComponent}>
          {#if activeComponent}
            ...
            {:else}
              ...
              <div style="border:1px solid var(--sd-border, #e5e7eb);border-radius:6px;overflow:hidden;display:flex;flex-direction:column;min-height:0">
                <div style="padding:8px 12px;display:flex;align-items:center;gap:8px;background:var(--sd-surface-raised, #fafafa);border-bottom:1px solid var(--sd-border, #e5e7eb)">
                  ...
                  <button style="margin-left:auto;font-size:10px;font-weight:600;padding:1px 8px;border-radius:3px;border:1px solid var(--sd-border, #d1d5db);background:var(--sd-surface-base, #fff);color:var(--sd-text, #1a1a1a);cursor:pointer" on:click={() => wakeOrbitor(entry.goal_id)}>Wake</button>
                </div>
      ...
      <div style="display:flex;flex-direction:column;gap:16px">
        <div style="display:flex;align-items:center;gap:12px">
          <button class="btn-prove" style="padding:6px 16px;font-size:13px;font-weight:600;border-radius:4px;cursor:{proofRunning?'wait':'pointer'};background:{proofRunning?'var(--sd-text-muted,#6b7280)':'var(--sd-accent,#2563eb)'}" disabled={proofRunning}
            on:click={() => runProofs(activeProof ? (PROOF_DOMAINS.find(d => d.label === activeProof)?.commands ?? []) : ALL_PROVE_COMMANDS)}>
        ...
        {#each (activeProof ? PROOF_DOMAINS.filter(d => d.label === activeProof) : PROOF_DOMAINS) as domain}
          <div>
            <div style="display:flex;align-items:center;gap:10px;margin-bottom:8px">
              ...
              {#if !activeProof}
                <button class="btn-action" disabled={proofRunning} on:click={() => runProofs(domain.commands)}>Run</button>
              {/if}
