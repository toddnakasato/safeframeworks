/**
 * Input builder for this renderer's SafeInput — config-driven view/edit field.
 *
 * Framework-agnostic DOM port of the react SafeInput implementation:
 * view mode (formatted display) + edit mode (native input per inputType).
 * Supports: text, multiline-text, number, currency, date, datetime,
 * checkbox, radio, toggle, picklist (single/multi/filter), lookup, slider,
 * rating, code, password, email, phone, url, search, time, color, file,
 * tags, autocomplete, masked, pin, range-date, signature, rich-text, address.
 *
 ...
 * covers both. Events: editmode, canceledit, change, textinput, navigate.
 *
 * Divergences from react (no third-party deps in vanilla DOM):
 *   - code: plain <textarea>/<pre> inside data-role="code-wrap" instead of Monaco.
 *   - markdown displayFormat: raw text inside data-role="markdown" (no react-markdown).
 */
...
/** Build the input field into a container. Returns the root for removal. */
export function createSafeInput(container: HTMLElement, config: ConfigBase, onEvent?: OnSafeEvent): HTMLElement {
    ...
    /** Wire blur-commit + keydown (Enter commits unless text/multiline; Escape cancels) like react. */
...
/**
 * Scan-and-mount for server-rendered hosts (Astro): build an input in every
 * div[data-input-config] not yet mounted.
 */
export function initSafeInputs(root: Document | HTMLElement = document): void {
    root.querySelectorAll<HTMLElement>("div[data-input-config]").forEach((host) => {
        if (host.dataset.inputMounted) return;
        host.dataset.inputMounted = "1";
        const config = JSON.parse(host.dataset.inputConfig!) as ConfigBase;
        createSafeInput(host, config);
    });
}
