/**
 * Timeline builder for this renderer's SafeTimeline — D3-powered timeline.
 *
 * Framework-agnostic DOM port of the react SafeTimeline (5 visual variants):
 *   default       vertical events list
 *   alternating   left/right zigzag
 *   horizontal    D3 SVG dots on a line
 *   swimlane      D3 SVG category lanes
 *   gantt         D3 SVG bars with time axis
 *
 * Structure + data-* attributes ONLY — paint lives in safestyles (data-driven
 * colors inline, as in react). Events: "select" (item click).
...
/** First DataSource's inline values from config.data (contract: Record keyed by name). */
...
/** Build the timeline into a container. Returns the root for removal. */
export function createSafeTimeline(container: HTMLElement, config: ConfigBase, onEvent?: OnSafeEvent): HTMLElement {
    ...
    /* --- D3 SVG variants: horizontal, swimlane, gantt ----------------- */
...
/**
 * Scan-and-mount for server-rendered hosts (Astro): build a timeline in every
 * div[data-timeline-config] not yet mounted.
 */
