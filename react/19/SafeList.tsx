...
function Pager({ page, totalPages, numbers, onPage }: { page: number; totalPages: number; numbers: boolean; onPage: (p: number) => void }) {
    ...
    return (
        <div data-role="list-pager">
            <button data-role="pager-btn" data-dir="prev" disabled={page <= 1} data-disabled={page <= 1 || undefined} onClick={() => onPage(page - 1)}>
                ‹ Previous
            </button>
            {numbers ? (
                ...
                <span data-role="pager-info">
                    {page} / {totalPages}
                </span>
            )}
            <button data-role="pager-btn" data-dir="next" disabled={page >= totalPages} data-disabled={page >= totalPages || undefined} onClick={() => onPage(page + 1)}>
                Next ›
            </button>
        </div>
    );
...
function usePager(count: number, pageSize: number, onEvent?: OnSafeEvent) {
    ...
    const slice = <T,>(items: T[]): T[] => (pageSize > 0 ? items.slice((clamped - 1) * pageSize, clamped * pageSize) : items);
    const go = (p: number) => {
    ...
    return (
        <div data-component="list" data-variant="columns" {...intentAttrs(meta)}>
            <div data-role="list-header" style={{ gridTemplateColumns: `repeat(${fields.length}, 1fr)` }}>
                {fields.map((f: any) => (
                    <span key={f.name} data-role="header-cell">
                        {f.label ?? f.name}
                    </span>
                ))}
            </div>
                ...
                {data.map((item, i) => {
                    ...
                    return (
                        <div key={i} data-role="list-item" tabIndex={0} role="listitem">
                            ...
                            {status && (
                                <span data-role="item-badge" data-accent={intent}>
                                    {status}
                                </span>
                            )}
                            {item[actionField] && (
                ...
                {slice(flat).map((node) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            <span data-role="item-icon">
                                <IconGlyph name={node[iconField]} size={16} />
                            </span>
    ...
    const renderNode = (node: any, level: number): React.ReactNode => {
            ...
            return (
                <div key={node.id} data-role="prop-group">
                    ...
                    >
                        ...
                        <span data-role="item-label" data-level={level}>
                            {node.name}
                        </span>
                    </div>
        ...
        return (
            <div key={id} data-role="prop-row" style={{ paddingLeft: `${level * 20 + 12}px` }}>
                ...
                {options ? (
                    <select data-role="prop-value" value={String(values[id] ?? "")} onChange={(e) => commit(id, e.target.value)}>
                        {options.map((o) => (
            ...
            <div data-role="gantt-scroll">
                <div data-role="gantt-header">
                    <span data-role="gantt-title">
                        <button data-role="gantt-nav" data-dir="prev" onClick={() => navigate(-1)}>
                            ‹
                        </button>
                        <span>{title}</span>
                        <button data-role="gantt-nav" data-dir="next" onClick={() => navigate(1)}>
                            ›
                        </button>
                    </span>
