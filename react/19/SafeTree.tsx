    ...
    return (
        <>
            ...
            >
                ...
                {iconField && node.record[iconField] && <span data-role="icon">{String(node.record[iconField])}</span>}
                <span data-role="label">{String(node.record[labelField] ?? "")}</span>
                {subtitleField && node.record[subtitleField] && <span data-role="subtitle">{String(node.record[subtitleField])}</span>}
                {badgeField && node.record[badgeField] && <span data-role="badge">{String(node.record[badgeField])}</span>}
            </div>
            {isExpanded &&
                node.children.map((child) => (
                    <TreeNodeRow key={child.id} node={child} config={config} expanded={expanded} onToggle={onToggle} onSelect={onSelect} selected={selected} />
                ))}
        </>
    );
}
...
export function SafeTree({ config, data, onEvent }: SafeTreeProps) {
    ...
    if (data.length === 0) {
        return (
            <div data-component="tree" data-role="empty">
                No items
            </div>
        );
    }
    ...
    return (
        <div data-component="tree" data-variant={variant} data-spacing={spacing} data-surface={surface}>
            {tree.map((node) => (
                <TreeNodeRow key={node.id} node={node} config={config} expanded={expanded} onToggle={handleToggle} onSelect={handleSelect} selected={selected} />
            ))}
        </div>
    );
}
