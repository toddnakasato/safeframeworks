/**
 * List builder for this renderer's SafeList — flexible collection of items.
 *
 * Framework-agnostic DOM port of the figma "Atom List Specs" (finAtomList):
 *   simple / icon       plain rows, optional leading icon + description
 *   selection           single (radio) or multiple (checkbox) pick list
 *   columns             grid rows driven by the data source schema
 *   files               file rows with icon, size and modified time
 *   actions             activity rows with status badge + action button
 *   hierarchy           expandable tree, paginated flat view
 *   property-grid       grouped name/value rows, editable values
 *   gantt               dark date header + activity range rows
 *
 * Structure + data-* attributes ONLY — paint lives in safestyles.
 * Events: select, toggle, expand, action, page, change, navigate.
...
/** Lucide svg element (or the raw name as text fallback, matching react). */
...
/** Build the list into a container. Returns the root for removal. */
...
/**
 * Scan-and-mount for server-rendered hosts (Astro): build a list in every
 * div[data-list-config] not yet mounted.
 */
