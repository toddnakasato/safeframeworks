/**
 * Toggle builder for this renderer's SafeToggle — config-driven on/off switch.
 *
 * Framework-agnostic DOM port of the react SafeToggle implementation:
 *   switch      single toggle with optional label/description/icon
 *   table       settings list with header row
 *   expandable  collapsible groups with child toggles
 *
 * Structure + data-* attributes ONLY — paint lives in safestyles.
 * Toggle state lives in builder closure. Events: "change" (toggle flipped),
 * "expand" (section opened/closed). Same data-role markup as the react JSX
 * implementation, so one stylesheet covers both.
 */
...
/** Build the toggle into a container. Returns the root for removal. */
...
/**
 * Scan-and-mount for server-rendered hosts (Astro): build a toggle in every
 * div[data-toggle-config] not yet mounted.
 */
