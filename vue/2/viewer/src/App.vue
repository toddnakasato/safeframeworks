...
onMounted(() => {
  ...
  listen('fs-change', async (_path: string) => {
    try {
      const raw = await invoke('read_file_content', { path: STATE_FILE });
  ...
  try {
    const results = await Promise.all(
      commands.map(async cmd => {
        try {
          const { invoke: tauriInvoke } = await import('@tauri-apps/api/core');
          const out = await tauriInvoke('safecli_run', { name: 'safedesk', args: ['prove', cmd] });
...
async function proveTicket(t: Ticket) {
  ...
  try {
    if (t.test) {
      ...
      const out = await tauriInvoke('safecli_run', { name: 'safedesk', args: parseCmd(t.test.command) });
