/**
 * Picker builder for this renderer's SafePicker — searchable list/card picker.
 *
 * Framework-agnostic DOM port of the react SafePicker implementation:
 *   default   — searchable list of rows
 *   card-grid — searchable grid of cards
 *
 * Structure + data-* attributes ONLY — paint lives in safestyles.
 * Search/filter state lives in builder closure; only the result list is
 * rebuilt on input so the search field keeps focus/value. Events:
 * "select" (row clicked), "filter" (dropdown changed). Same data-role
 * markup as the react JSX implementation, so one stylesheet covers both.
 */
...
/** Build the picker into a container. Returns the root for removal. */
export function createSafePicker(container: HTMLElement, config: ConfigBase, onEvent?: OnSafeEvent): HTMLElement {
    ...
    /* ---------------- search form (built once; survives re-renders) -------- */
    ...
    if (showSearch) {
        ...
        if (filterField && filterOptions.length > 0) {
            ...
            select.addEventListener("change", () => {
                ...
                onEvent?.(createSafeEvent("picker", "filter", { field: filterField, value: select.value }));
                renderResults();
            });
    ...
    /* ---------------- results (rebuilt on search/filter) ------------------- */
    ...
    function renderResults() {
        ...
        filtered.forEach((row, i) => {
            if (isCardGrid) {
                ...
                card.onclick = () => onEvent?.(createSafeEvent("picker", "select", { row, index: i }));
                buildRows(card, row);
                results.appendChild(card);
            } else {
                const li = el("li", "list-item");
                li.setAttribute("data-spacing", spacing);
                li.onclick = () => onEvent?.(createSafeEvent("picker", "select", { row, index: i }));
                buildRows(li, row);
...
/**
 * Scan-and-mount for server-rendered hosts (Astro): build a picker in every
 * div[data-picker-config] not yet mounted.
 */
