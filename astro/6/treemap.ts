/**
 * Treemap builder for this renderer's SafeTreemap — D3 nested rectangles.
 *
 * Framework-agnostic DOM port of the react JSX implementation:
 * flat data with id/parent (stratify) or simple label/value list (hierarchy),
 * rect cells sized by value with conditional labels.
 *
 * Structure + data-* attributes mirror react exactly; the only inline styles
 * are those react itself sets inline (svg sizing). Events: "select".
 */
...
/** Build the treemap into a container. Returns the root for removal. */
...
/**
 * Scan-and-mount for server-rendered hosts (Astro): build a treemap in every
 * div[data-treemap-config] not yet mounted.
 */
