/**
 * Chart.js builder for this renderer's SafeChart.
 *
 * Framework-agnostic: maps ChartMetadata (safecontracts) → Chart.js config.
 * SafeChart calls createSafeChart(canvas, config, onEvent).
...
/** Resolve "var(--sd-x, fallback)" to a concrete color — canvas cannot read CSS vars. */
...
/** Map ConfigBase + data → Chart.js configuration. */
...
/** Create a Chart.js instance on a canvas. Fires SafeEvent "click" per COMPONENT_EVENTS. */
export function createSafeChart(canvas: HTMLCanvasElement, config: ConfigBase, onEvent?: OnSafeEvent): Chart {
    ...
    cfg.options = {
        ...
        onClick: (_evt: any, elements: any[]) => {
            ...
            const el = elements[0];
            const data = chartData(config);
            onEvent(createSafeEvent("chart", "click", {
                index: el.index,
...
/**
 * Scan-and-mount for server-rendered hosts (Astro): instantiate every
 * canvas[data-chart-config] not yet mounted.
 */
